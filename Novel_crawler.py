...
def NovelSearch(searchStr):
    ...
    #print(searchStr_url)
    search_url = 'http://www.biquge.tv/modules/article/search.php?searchkey='+searchStr_url
    
    response = requests.get(search_url,headers=headers)
    print(response.text)
    response.encoding = 'gbk'
    soup=BeautifulSoup(response.text,"html.parser")
    Search_res_active = soup.select('body #wrapper #main #content tbody tr')
    ...
    if Search_res_active_len==0 or 0==len(Search_res_page):
        ...
        if len(Search_res_one)!=0 :
            ...
            if len(one)>27 or one[0:26] == "https://www.52bqg.com/book_":
                novel_name= soup.select('body #wrapper #main #content tbody h1')[0].text
